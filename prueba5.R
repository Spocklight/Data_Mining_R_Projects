...
#A partir de aquí seguimos con datos_training_med para ver si mejoramos los resultados obtenidos anteriormente
...
datos_training_med$ContratoBI <- as.character(datos_training_med$Contrato)
datos_training_med$ContratoBI[datos_training_med$ContratoBI == "One year"] <- "+Month"
datos_training_med$ContratoBI[datos_training_med$ContratoBI == "Two year"] <- "+Month"
datos_training_med$ContratoBI <- as.factor(datos_training_med$ContratoBI)

questionr::freq(datos_training_med$Contrato)
questionr::freq(datos_training_med$ContratoBI)

#Creamos un par de variables de control:

datos_training_med$aleatorio<-runif(nrow(datos_training_med))

datos_training_med$aleatorio2<-runif(nrow(datos_training_med))

str(datos_training_med)
...
unique(datos_training_med$prop_missings) #Al no haber 6 valores diferentes no aparece prop_missings en la relación
...
datos_training_med$prop_missings_cat <- as.character(datos_training_med$prop_missings)

datos_training_med$prop_missings_cat[datos_training_med$prop_missings_cat != "0"]  <- "Con Missings"
datos_training_med$prop_missings_cat[datos_training_med$prop_missings_cat == "0"] <- "Sin Missings"
...
m1<-mosaico_targetbinaria(datos_training_med$Contrato,varObj,"Contrato")  #Tablas de contingencias en gráfico 
...
bx1<-boxplot_targetbinaria(datos_training_med$Antig.fc.edad,varObj,"Antig.")  
bx2<-boxplot_targetbinaria(datos_training_med$FacturaMes,varObj,"Factura_mes")
bx3<-boxplot_targetbinaria(datos_training_med$FacturaTotal,varObj,"Factura_tot")

h1<-barras_targetbinaria(datos_training_med$Contrato,varObj,"Contrato") 
h2<-barras_targetbinaria(datos_training_med$MetodoPago,varObj,"Pago") 
h3<-barras_targetbinaria(datos_training_med$Int_serv,varObj,"Servicio") 
